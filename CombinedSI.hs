...
-}
read_astro :: {-(Ord v, Fractional v) => -}VisData -> IO (Grid3D Float)
read_astro d@(From xr yr zr t sp)
    = do let basename = show d
         let summary = basename ++ ".summary"
         let dim = Z :. range_size zr :. range_size yr :. range_size xr
         let size = (range_size zr) * (range_size yr) * (range_size xr)
         h <- openBinaryFile (basename++".dat") ReadMode    
         bs <- Fast.hGetContents h
         --vs <- hGetContents h
         -- let vs = map (bytesToValues bs) [0..size-1]
         haveSummary <- fileExist summary    
         [min, max] <- return $ [0,35000]--if haveSummary
                          --then do v <- Fast.readFile summary
                          --        return [bytesToValues 0 v, bytesToValues 1 v]
                          --else return $ compute_bounds vs
         return $ Grid basename (show sp) dim t min max bs
         --return $ Grid basename (show sp) dim t min max $ Str.fromStream $ byteStreamToValues $ Str.toStream vs
{-# INLINE read_astro #-}

compute_bounds :: (Ord v, Fractional v) => [v] -> [v]
compute_bounds vs = [minimum vs, maximum vs]
...
{-    
charsToWord32 (d:c:b:a:rest) = s: charsToWord32 rest
  where s = (a32 .|. b32 .|. c32 .|. d32)
          a32 :: Word32 = fromIntegral (fromEnum a) `shiftL` 24
          b32 :: Word32 = fromIntegral (fromEnum b) `shiftL` 16
          c32 :: Word32 = fromIntegral (fromEnum c) `shiftL`  8
          d32 :: Word32 = fromIntegral (fromEnum d)
-}
--foreign import ccall "floatOf" c_Word32ToFloat :: Word32 -> Float
        -- imported from fixedPrecision.h


--byteStreamToValues :: Str.Stream Word8 -> Str.Stream Float
{-# INLINE_STREAM byteStreamToValues #-}
{-
byteStreamToValues (Str.Stream next s n c) 
    = Str.Stream next' s n ("byteStreamToValues" `Str.sArgs` c)
      where
        {-# INLINE next' #-}
        next' s = case next s of
                    Str.Done       -> Str.Done
                    Str.Skip s'    -> Str.Skip s'
                    Str.Yield a s' -> case next s' of
                                        Str.Done        -> byteError
                                        Str.Skip _      -> byteError
                                        Str.Yield b s'' -> case next s'' of
                                                             Str.Done         -> byteError
                                                             Str.Skip _       -> byteError
                                                             Str.Yield c s''' -> case next s''' of
                                                                                   Str.Done          -> byteError
                                                                                   Str.Skip _        -> byteError
                                                                                   Str.Yield d s'''' -> Str.Yield (realToFrac val) s''''
                                                                                                        where
                                                                                                          a16 :: Int16 = fromIntegral $ fromEnum a `shiftL` 8
                                                                                                          b16 :: Int16 = fromIntegral $ fromEnum b 
                                                                                                          c16 :: Int16 = fromIntegral $ fromEnum c `shiftL`  8
                                                                                                          d16 :: Int16 = fromIntegral $ fromEnum d    
                                                                                                          exp = a16 .|. b16
                                                                                                          man = c16 .|. d16
                                                                                                          val | exp < 0   = toInteger man % (10 ^ negate (toInteger exp))
                                                                                                              | otherwise = toInteger man * (10^exp) % 1  
                                      where
                                        byteError = error "byteStreamToValues: Expecting groups of 4 bytes"
{-# INLINE byteStreamToValues #-}
-}
    ...
    | otherwise      = (realToFrac val):bytesToValues post
                       where
                           ...
                           val | exp < 0   = toInteger man % (10 ^ negate (toInteger exp))
                               | otherwise = toInteger man * (10^exp) % 1  
{-
bytesToValues :: Fractional a => Fast.ByteString -> Int -> a
bytesToValues bs !i
...
iso3D th grid = iso' 0 0 0 0
                where
                    vals = values grid
                    plane = xsz * ysz
                    (Z :. zs :. ys :. xs) = shape grid
                    xsz = fromIntegral xs
                    ysz = fromIntegral ys
                    zsz = fromIntegral zs
                    iso' !k !j !i !off
                      | k == zsz-1 = []
                      | j == ysz-1 = iso' (k+1) 0 0 (off+xsz)
                      | i == xsz-1 = iso' k (j+1) 0 (off+1)
                      | otherwise  = let !i1 = i + 1
                                         !j1 = j + 1
                                         !k1 = k + 1
                                         !ofp = off + plane
                                         !av = getv off
                                         !bv = getv (off+1)     -- arr !: (Z :. k  :. j  :. i1)
                                         !cv = getv (off+1+xsz) -- arr !: (Z :. k  :. j1 :. i1)
                                         !dv = getv (off  +xsz) -- arr !: (Z :. k  :. j1 :. i )
                                         !ev = getv ofp         -- arr !: (Z :. k1 :. j  :. i )
                                         !fv = getv (ofp+1)     -- arr !: (Z :. k1 :. j  :. i1)
                                         !gv = getv (ofp+1+xsz) -- arr !: (Z :. k1 :. j1 :. i1)
                                         !hv = getv (ofp  +xsz) -- arr !: (Z :. k1 :. j1 :. i )
{-                                         
                                         !ofp = off + plane
                                         !av = vals `Fast.index` off
                                         !bv = vals `Fast.index` (off+1)     -- arr !: (Z :. k  :. j  :. i1)
                                         !cv = vals `Fast.index` (off+1+xsz) -- arr !: (Z :. k  :. j1 :. i1)
                                         !dv = vals `Fast.index` (off  +xsz) -- arr !: (Z :. k  :. j1 :. i )
                                         !ev = vals `Fast.index` ofp         -- arr !: (Z :. k1 :. j  :. i )
                                         !fv = vals `Fast.index` (ofp+1)     -- arr !: (Z :. k1 :. j  :. i1)
                                         !gv = vals `Fast.index` (ofp+1+xsz) -- arr !: (Z :. k1 :. j1 :. i1)
                                         !hv = vals `Fast.index` (ofp  +xsz) -- arr !: (Z :. k1 :. j1 :. i )
-}
                                         ir  :: GL.GLfloat = realToFrac i
                                         jr  :: GL.GLfloat = realToFrac j
                                         kr  :: GL.GLfloat = realToFrac k
                                         i1r :: GL.GLfloat = realToFrac i1
                                         j1r :: GL.GLfloat = realToFrac j1
                                         k1r :: GL.GLfloat = realToFrac k1
                                         ts = mc_case $ Cell_8 (av > th) (bv > th) (cv > th) (dv > th) (ev > th) (fv > th) (gv > th) (hv > th)   
                                         getv index = let start = index * 4
                                                          a = vals `Fast.index` start
                                                          b = vals `Fast.index` (start + 1)
                                                          c = vals `Fast.index` (start + 2)
                                                          d = vals `Fast.index` (start + 3)
                                                          a16 :: Int16 = fromIntegral a `shiftL` 8
                                                          b16 :: Int16 = fromIntegral b 
                                                          c16 :: Int16 = fromIntegral c `shiftL`  8
                                                          d16 :: Int16 = fromIntegral d    
                                     ...
                                     in  
                                         if ts == [] then iso' k j (i+1) (off+1)
                                                                 ...
                                                                 (Cell_8 (Vertex3 ir  jr  kr )
                                                                         ...
                                                                         (Vertex3 i1r j1r k1r)
                                                                         (Vertex3 ir  j1r k1r)
                                                                 )) ts) : iso' k j (i+1) (off+1) 
                                          
--surf_geom :: (Interp a, InvInterp a, Interp g, Cell c v, Enum v) =>
--    a -> c a -> c g -> (v,v) -> g
{-# SPECIALISE surf_geom :: Float -> Cell_8 Float -> Cell_8 (Vertex3 GL.GLfloat) -> (MyVertex,MyVertex) -> Vertex3 GL.GLfloat
 #-}
...
main :: IO ()
main = do { ds :: Grid3D Float {-Double-} <- read_astro (from4 60 SpG) --(from4 60 G) 
          ; let dminv = minv ds
          ; let dmaxv = maxv ds  
